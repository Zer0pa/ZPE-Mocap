...
def run_core_benchmarks(seed: int, search_library_size: int = 10_000, query_count: int = 120) -> BenchmarkBundle:
    ...
    for i, clip in enumerate(clips):
        ...
        raw_float_bytes = clip.angles_deg.astype(np.float32).tobytes()
    ...
    for idx in range(search_library_size):
        label = ACTION_LABELS[idx % len(ACTION_LABELS)]
        tokens = _fast_token_stream(label=label, length=120, seed=seed + idx)
        search_index.add(f"lib_{idx:05d}", tokens, label)

    for q in range(query_count):
        label = ACTION_LABELS[q % len(ACTION_LABELS)]
        labels.append(label)
        tokens = _fast_token_stream(label=label, length=120, seed=seed + 100_000 + q)
